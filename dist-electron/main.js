...
const gitInstances = /* @__PURE__ */ new Map();
function getGit(projectPath) {
  if (!gitInstances.has(projectPath)) {
    gitInstances.set(projectPath, esm_default(projectPath));
  }
...
function registerGitIpc() {
  electron.ipcMain.handle("git:status", async (_event, projectPath) => {
    ...
    try {
      ...
      return {
        ...
        staged: status.staged.map((f2) => ({ path: f2, status: "modified" })),
        modified: status.modified.map((f2) => ({ path: f2, status: "modified" })),
  ...
  electron.ipcMain.handle("git:log", async (_event, projectPath, maxCount = 50) => {
    const git = getGit(projectPath);
    const log = await git.log({ maxCount });
    return log.all.map((c2) => ({
      hash: c2.hash,
      shortHash: c2.hash.substring(0, 7),
      author: c2.author_name,
      email: c2.author_email,
      date: c2.date,
      message: c2.message
    }));
  });
  ...
  electron.ipcMain.handle("git:add", async (_event, projectPath, files) => {
    const git = getGit(projectPath);
    await git.add(files);
  });
  ...
  electron.ipcMain.handle("git:push", async (_event, projectPath) => {
    ...
    return { status: "ok", summary: ((_a2 = result.pushed) == null ? void 0 : _a2.map((p) => `${p.local} -> ${p.remote}`).join(", ")) || "Nothing to push" };
  });
  electron.ipcMain.handle("git:checkout", async (_event, projectPath, branch) => {
    const git = getGit(projectPath);
  ...
  electron.ipcMain.handle("git:show", async (_event, projectPath, hash) => {
    const git = getGit(projectPath);
